    ...
    return (
      <>
        <div className="max-h-screen w-fit select-none">
          <Create create={props.create} setCreate={props.setCreate} setActive={props.setActive} supabase={props.supabase} theme={props.theme} />
          <Search search={props.search} viewport={props.viewport} />
          <div id="Sidebar" className={"fixed left-0 grid gap-4 " + props.theme.primary + (props.viewport == "Web" && " bottom-0 z-10 h-full w-72 grid-flow-row transition-all duration-200 border-r-[1px] ") + (props.viewport == "Tab" && " top-0 h-full w-16 grid-flow-row transition-all duration-200 border-r-[1px] ") + (props.viewport == "Mobile" && " bottom-0 h-12 w-screen grid-flow-col border-t-[1px] ") + (props.create && " opacity-30 ")}>
