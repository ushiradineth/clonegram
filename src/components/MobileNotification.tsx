...
const MobileNotification = (props: itemType) => {
  ...
  const NoNotifications = (props: { text: string }) => {
    return (
      <div className={"flex w-full flex-col items-center justify-center rounded-2xl p-8 " + data?.theme?.primary}>
  ...
  const NotificationContainer = (props: { element: any; index: number; setNotification: any }) => {
    ...
    return (
      <div key={props.index} className="flex h-fit w-fit items-center justify-center gap-4 p-4">
        ...
        {(props.element.type !== "PostLike") && <Image onClick={() => onClickHandler("/post/" + props.element.post.id)} className={"h-[50px] w-[50px] cursor-pointer rounded-2xl  object-cover"} src={props.element.post.imageURLs[0] || "https://hmgdlvdpchcrxwiqomud.supabase.co/storage/v1/object/public/clonegram/Assets/image-placeholder.png"} alt={String(props.index)} width={200} height={200} />}
      </div>
    );
  };
