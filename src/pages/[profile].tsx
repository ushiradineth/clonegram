...
const Profile = (props: itemType) => {
  ...
  const doubleQuery = trpc.user.getUsersByHandle.useQuery({ handle_1: session?.user?.handle || "0", handle_2: String(profile) }, { retry: false, refetchOnWindowFocus: false, enabled: Boolean(status === "authenticated" && session?.user?.handle !== String(profile) && !router.query.user) });

  const page = status === "unauthenticated" ? singleQuery.data : session?.user?.handle === String(profile) ? singleQuery.data : doubleQuery.data?.at(0);
  const user = status === "unauthenticated" ? singleQuery.data : session?.user?.handle === String(profile) ? singleQuery.data : doubleQuery.data?.at(1);
  ...
  useEffect(() => {
    if (doubleQuery.isSuccess || singleQuery.isSuccess) {
      ...
      page?.following.forEach((element) => {
      ...
      page?.followers.forEach((element) => {
  ...
  if (doubleQuery.isError || singleQuery.isError) {
  ...
  if (status === "unauthenticated" ? singleQuery.isLoading : session?.user?.handle === String(profile) ? singleQuery.isLoading : doubleQuery.isLoading || doubleQuery.isFetching) return <Spinner viewport={props.viewport} theme={props.theme} />;
              ...
              <div id="headline" className={"mb-4 mt-6 ml-4 grid grid-flow-row " + (props.viewport != "Mobile" && " h-fit gap-3 ")}>
                <div id="user-info">
                  <div className="flex items-center gap-4">
                    ...
                    <button id="cta" onClick={() => (session ? (session?.user?.id === page?.id ? setEditProfile(true) : isFollowing ? unfollowFunc() : followFunc()) : router.push("/"))} className={"cursor-pointer rounded-[4px] border-2 py-1 px-2 text-xs font-semibold " + (props.viewport == "Mobile" && " hidden ")}>
