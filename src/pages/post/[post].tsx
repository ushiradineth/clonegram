      ...
      <div className={"flex w-full flex-col overflow-auto border-zinc-600 " + (data?.viewport === "Mobile" ? " h-fit border-t " : " h-[70%] border-b ")}>
        ...
        {post.data?.comments.length ? (
          post.data.comments
            .slice(0)
            .reverse()
            .map((element, index) => <Comment key={index} comment={element} post={post} setDeleteCommentID={setDeleteCommentID} setDeleteCommentMenu={setDeleteCommentMenu} setLikeUsers={setLikeUsers} setLikesMenu={setLikesMenu} />)
        ) : (
    ...
    return (
      <div className={"grid transform place-items-center " + (data?.viewport === "Mobile" ? " h-fit max-h-[700px] w-[90%] sm:w-full " : " h-[700px] rounded-l-2xl ") + data?.theme?.primary}>
        <div className={"flex h-fit max-h-screen w-fit max-w-[300px] items-center justify-center transition-all duration-300"}>
          ...
          <Image src={post.data?.imageURLs[imageIndex] || "https://hmgdlvdpchcrxwiqomud.supabase.co/storage/v1/object/public/clonegram/Assets/image-placeholder.png"} key="image" className="h-full w-full object-cover" height={1000} width={1000} alt={"images"} priority />
        </div>
      </div>
    );
  };
